...
Maze::Maze(int n, int m, int startX, int startY): n(n), m(m), startX(startX), startY(startY)
{
    isGenerated = isLoaded = false;
    delta[up] = make_pair(-1, 0);
    delta[down] = make_pair(1, 0);
...
void Maze::sfs() //strange first search
{
    ...
    while (!q.empty())
    {
        ...
        while (!possibleDirections.empty())
        {
            ...
            {
                makeEdge(x, y, curDirection);
                used[x + delta[curDirection].X][y +delta[curDirection].Y] = true;
                q.push_back(make_pair(x + delta[curDirection].X, y +delta[curDirection].Y));
            }
        }
    }
}
    ...
    while (!q.empty())
    {
        ...
        while (!possibleDirections.empty())
        {
            ...
            if (!used[x + delta[curDirection].X][y + delta[curDirection].Y])
            {
                makeEdge(x, y, curDirection);
                used[x + delta[curDirection].X][y +delta[curDirection].Y] = true;;
                q.push(make_pair(x + delta[curDirection].X, y +delta[curDirection].Y));
            }
        }
    }
}
